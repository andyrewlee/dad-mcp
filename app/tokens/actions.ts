...
export async function createAccessToken(userId: string): Promise<{ success: boolean; newToken?: string; error?: string }> {
...
export async function deleteAccessToken(tokenId: string): Promise<{ success: boolean; error?: string }> {
  const supabase = await createClient();
